...
async function filterStep(
  ...
  const systemPrompt = [
    ...
    "",
    `Today's date is ${today}. Use this whenever the question contains a`,
    `relative time expression ("last 3 weeks", "since March", "past quarter").`,
    "",
    "Rules for constructing scope_of_exploration:",
    "  1. If the question contains a HARD FILTER — either a time range",
    "     (e.g. 'last 3 weeks', 'since 2026-03-01', 'in Q1') or a categorical",
    "     filter derivable from metadata (customer tier, meeting_type,",
    "     product_discussed, specific customer name, participant role, etc.) —",
    "     scope_of_exploration MUST include EVERY document whose metadata",
    "     satisfies that filter, and MUST NOT include any document that",
    "     violates it. Do not prune inside the filter set; the sub-agents will.",
    "  2. If the question has no hard filter, be inclusive — include every",
    "     document that could plausibly contain relevant content.",
    "  3. You see ONLY metadata, never bodies. For content-shaped filters",
    "     ('about pricing', 'discussed integrations'), you can only use",
    "     metadata cues (e.g. `topics_tagged`, `meeting_type`). When unsure,",
    "     INCLUDE the document.",
    "  4. Return doc IDs verbatim from the metadata index.",
  ].join("\n");
